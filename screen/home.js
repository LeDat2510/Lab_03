...
const Home = ({ navigation }) => {
        ...
        const handleSelectedItem = () => {
            {
                userLogin.role == "admin" ? (
                    navigation.navigate('RouterServices', { screen: 'ServiceDetail', params: { item } })
                ) : (
                    navigation.navigate('RouterCustomerServices', { screen: 'Appointment', params: { item } })
                )
            }
        }
        return (
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Image source={require('../image/logo.png')} style={{ height: '20%', width: '50%', alignSelf: 'center', resizeMode: 'contain' }}></Image>
            <View style={{ flexDirection: 'row', alignSelf: 'center', justifyContent: 'space-between', width: '90%' }}>
                ...
                {
                    userLogin.role == "admin" && (
                        <TouchableOpacity onPress={() => navigation.navigate('RouterServices', { screen: 'AddNewService' })}>
